...
struct DashboardResource {
    ...
    func getSubscribedMealDetail(urlStr:String, request: SubscribedMealDetailRequest, onSuccess:@escaping(SubscribedMealDetailResponse) -> Void, onError:@escaping(BatchError) -> Void) {
        do {
            let requestBody = try JSONEncoder().encode(request)
            do {
                let decodedRequest = try JSONDecoder().decode(SubscribedMealDetailRequest.self, from: requestBody)
                print(decodedRequest)
            } catch {
                print("Error decoding request body: \(error)")
            }
            let huRequest = HURequest(url: URL(string: urlStr)!, method: .post, requestBody: requestBody)
            HttpUtility.shared.request(huRequest: huRequest, isAuthorization: false, resultType: SubscribedMealDetailResponse.self) { (result) in
                switch result{
                case .success(let response):
                    onSuccess(response!)
                case .failure(let error):
    ...
    // Macro Detail API with Post Request.
    func getMacroList(urlStr:String, request: MacroRequest, onSuccess:@escaping(MacroResponse) -> Void, onError:@escaping(BatchError) -> Void) {
        do {
            let requestBody = try JSONEncoder().encode(request)
            do {
                let decodedRequest = try JSONDecoder().decode(MacroRequest.self, from: requestBody)
                print(decodedRequest)
            } catch {
                print("Error decoding request body: \(error)")
            }
            let huRequest = HURequest(url: URL(string: urlStr)!, method: .post, requestBody: requestBody)
            HttpUtility.shared.request(huRequest: huRequest, isAuthorization: false, resultType: MacroResponse.self) { (result) in
                switch result{
                case .success(let response):
                    onSuccess(response!)
                case .failure(let error):
