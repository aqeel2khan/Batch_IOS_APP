//
//  BUserDeliveryDetailResource.swift
//  Batch
//
...
struct BUserDeliveryDetailResource {
    
    func getDeliveryAddress(onSuccess:@escaping(GetProfileResponse) -> Void, onError:@escaping(BatchError) -> Void){
    ...
    func updateDeliveryAddress(request: UpdateProfileRequest, onSuccess:@escaping(UpdateProfileResponse) -> Void, onError:@escaping(BatchError) -> Void){
        do {
    ...
    func addDeliveryAddress(request: UpdateProfileRequest, onSuccess:@escaping(UpdateProfileResponse) -> Void, onError:@escaping(BatchError) -> Void){
        do {
            let requestBody = try JSONEncoder().encode(request)
            let huRequest = HURequest(url: URL(string: API.updateProfileDetail)!, method: .put, requestBody: requestBody)
